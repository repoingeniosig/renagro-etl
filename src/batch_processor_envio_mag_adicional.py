"""
Procesador de lotes para envío de tablas adicionales a MAG
Soporta capacitacion, comunicacion y produccion con configuración por target.
"""
...
TARGET_REMOTE_ID_COLUMN = {
    'capacitacion': 'id_capacitacion_creada',
    'comunicacion': 'id_comunicacion_creada',
    'produccion': 'id_produccion_creada'
}


class BatchProcessorEnvioMAGAdicional:
    """Procesador de lotes para tablas adicionales por target"""
        ...
        for main_row in main_table_data:
            ...
            if not control_pk:
                envio_mag_logger.warning(
                    f"[BatchProcessorEnvioMAGAdicional:{self.target_name}] "
            ...
            try:
                ...
                envio_mag_logger.error(
                    f"[BatchProcessorEnvioMAGAdicional:{self.target_name}] {error_msg}",
                    exc_info=True
                )
            ...
            for message in json_list:
                try:
                    ...
                    if success:
                        ...
                        envio_mag_logger.error(
                            f"[BatchProcessorEnvioMAGAdicional:{self.target_name}] "
                            f"Error publicando mensaje control_id={message['control_id']}"
                        )
                        self._mark_as_error(
                            message['control_id'],
                            "Error publicando mensaje a cola RabbitMQ"
                        )
                except Exception as e:
                    envio_mag_logger.error(
                        f"[BatchProcessorEnvioMAGAdicional:{self.target_name}] "
                        f"Excepción publicando: {e}",
                        exc_info=True
                    )
                    self._mark_as_error(
                        message['control_id'],
                        f"Excepción publicando: {str(e)[:200]}"
                    )
...
# Instancias globales por target
batch_processor_capacitacion = BatchProcessorEnvioMAGAdicional('capacitacion')
batch_processor_comunicacion = BatchProcessorEnvioMAGAdicional('comunicacion')
batch_processor_produccion = BatchProcessorEnvioMAGAdicional('produccion')


async def process_all_additional_targets() -> int:
    """Procesa los 3 targets adicionales secuencialmente"""
    total = 0
    for target_name in ['capacitacion', 'comunicacion', 'produccion']:
        count = await process_additional_target(target_name)
        total += count
    return total


async def process_additional_target(target_name: str) -> int:
    """Procesa un target adicional específico"""
    processors = {
        'capacitacion': batch_processor_capacitacion,
        'comunicacion': batch_processor_comunicacion,
        'produccion': batch_processor_produccion
    }

    processor = processors.get(target_name)
    if not processor:
        envio_mag_logger.error(f"Target adicional desconocido: {target_name}")
        return 0

    return await processor.process_all_pending()
