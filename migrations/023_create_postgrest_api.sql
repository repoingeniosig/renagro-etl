-- Migration: Create PostgREST API schema, roles, and functions
-- Description: Exponer boletas aprobadas y enviadas vía PostgREST con autenticación JWT
-- Schema: api (dedicado para PostgREST)
-- Date: 2026-02-26
...
-- Rol de conexión de PostgREST (LOGIN, no hereda permisos por defecto)
DO $$
BEGIN
...
-- Rol anónimo (sin acceso a datos)
DO $$
BEGIN
...
-- Rol autenticado vía JWT (lectura de datos)
DO $$
BEGIN
...
-- PostgREST necesita poder cambiar a estos roles
GRANT web_anon TO authenticator;
GRANT api_user TO authenticator;

-- ============================================================================
-- 3. PERMISOS DE SCHEMA
-- ============================================================================
GRANT USAGE ON SCHEMA api TO web_anon, api_user;
GRANT USAGE ON SCHEMA sc_renagro_mag TO api_user;
GRANT SELECT ON ALL TABLES IN SCHEMA sc_renagro_mag TO api_user;

-- Permisos para tablas futuras en sc_renagro_mag
ALTER DEFAULT PRIVILEGES IN SCHEMA sc_renagro_mag GRANT SELECT ON TABLES TO api_user;

-- ============================================================================
-- 4. FUNCIÓN PRINCIPAL: get_boletas_aprobadas
--    Paginación keyset: WHERE bol_id > p_after_id ORDER BY bol_id ASC
...
AS $$
  SELECT (
    -- ======== PARTE 1: Identidad, persona, ubicación, contacto, servicios, beneficios, asociación ========
    jsonb_build_object(
      'id', b.bol_id,
    ...
    jsonb_build_object(
      'respuestaNegativaMiembro', b.bol_respuesta_negativa_miembro,
      'motosierra', b.bol_motosierra,
    ...
    jsonb_build_object(
      ...
      'bombaAgua', b.bol_bomba_agua,
      'bombaMochila', b.bol_bomba_mochila,
      ...
      'compradorCuyesConejos', b.bol_comprador_cuyes_conejo,
      ...
      'sitioCuyesConejos', b.bol_sitio_cuyes_conejo,
    ...
    jsonb_build_object(
      ...
      'necesidadConocimientos', b.bol_necesidad_conocimiento,
      ...
      'prestamoRazonNoGestiono', b.bol_prestamo_razon_no_gestiono,
...
COMMENT ON FUNCTION api.get_boletas_aprobadas IS 'Retorna boletas aprobadas y enviadas con estructura JSON anidada completa. Paginación keyset: usar p_after_id con el último id recibido. Máximo 100 registros por página.';

-- ============================================================================
-- 5. FUNCIÓN DE CONTEO ESTIMADO
-- ============================================================================
CREATE OR REPLACE FUNCTION api.get_boletas_aprobadas_count()
RETURNS JSON
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT json_build_object(
    'estimatedTotal', (
      SELECT COUNT(*)
      FROM sc_renagro_mag.control_envios_boletas ceb
      WHERE ceb.estado_etl = 'APROBADO'
        AND ceb.envio_datos_procesados = 'ENVIADO'
    ),
    'maxPerPage', 100
...
COMMENT ON FUNCTION api.get_boletas_aprobadas_count IS 'Retorna el conteo de boletas aprobadas y enviadas disponibles, junto con el máximo de registros por página.';

-- ============================================================================
-- 6. PERMISOS SOBRE FUNCIONES
-- ============================================================================
GRANT EXECUTE ON FUNCTION api.get_boletas_aprobadas(BIGINT, INT) TO api_user;
GRANT EXECUTE ON FUNCTION api.get_boletas_aprobadas_count() TO api_user;

-- Revocar acceso anónimo a las funciones (solo usuarios autenticados)
REVOKE EXECUTE ON FUNCTION api.get_boletas_aprobadas(BIGINT, INT) FROM web_anon;
REVOKE EXECUTE ON FUNCTION api.get_boletas_aprobadas_count() FROM web_anon;

-- ============================================================================
-- 7. ÍNDICE PARA OPTIMIZAR EL JOIN
-- ============================================================================
CREATE INDEX IF NOT EXISTS idx_control_boletas_aprobado_enviado
  ON sc_renagro_mag.control_envios_boletas (uuid_boleta)
  WHERE estado_etl = 'APROBADO' AND envio_datos_procesados = 'ENVIADO';
