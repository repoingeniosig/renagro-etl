"""
Worker para envío de tablas adicionales a API remota RENAGRO.
Consume cola QUEUE_ENVIO_MAG_ADICIONAL_SEND y actualiza estado en tablas de control.
"""
import asyncio
...
from .api_sender_envio_mag_adicional import api_sender_envio_mag_adicional


class EnvioMagSenderWorkerAdicional:
    """Worker de envío paralelo para tablas adicionales"""
    ...
    async def send_json_task(self, message: Dict[str, Any]):
        async with self.semaphore:
            ...
            try:
                ...
                success, status_code, error_msg, remote_id = await api_sender_envio_mag_adicional.send_json(
                ...
                if success:
                    ...
                    envio_mag_logger.info(
                        f"[EnvioMagSenderWorkerAdicional] ✅ Enviado: "
                    ...
                    envio_mag_logger.error(
                        f"[EnvioMagSenderWorkerAdicional] ❌ Error enviando: "
                        f"target={target_type}, control_id={control_id}, status={status_code}, error={error_msg}"
                    )

            except Exception as e:
                envio_mag_logger.error(
                    f"[EnvioMagSenderWorkerAdicional] Error inesperado: control_id={control_id}, {e}",
                    exc_info=True
                )
                await self.update_control_status(
                    control_table=control_table,
    ...
    def print_stats(self):
        ...
        envio_mag_logger.info("ESTADÍSTICAS DE ENVÍO ADICIONAL")
        envio_mag_logger.info("=" * 80)
...
envio_mag_sender_worker_adicional = EnvioMagSenderWorkerAdicional()
